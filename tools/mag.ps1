...
function Task-CI {
  ...
  try {
    ...
    & $NPM run -s typecheck
    if ($LASTEXITCODE -ne 0) { throw "typecheck saiu com código $LASTEXITCODE" }
    & $NPM run -s lint
    if ($LASTEXITCODE -ne 0) { throw "lint saiu com código $LASTEXITCODE" }
    & $NPM run -s test
    if ($LASTEXITCODE -ne 0) { throw "test saiu com código $LASTEXITCODE" }
  } finally {
    Pop-Location
  }
  Task-Report -FromCI
