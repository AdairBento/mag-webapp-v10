...
r.get("/", async (_req, res) => {
  const items = await prisma.notificationMessage.findMany({ orderBy: { createdAt: "desc" } as any });
  res.json(items);
});

r.post("/", async (req, res) => {
  try {
    const { tenantId, channel, kind, type, subject, title, body, content, active } = req.body ?? {};
