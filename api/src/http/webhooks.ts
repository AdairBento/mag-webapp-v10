  ...
  try {
    ...
    for (const entry of entries) {
      ...
      for (const c of changes) {
        ...
        await prisma.notificationMessage.updateMany({ where: { externalId }, data: { status: st, deliveredAt: st === 'delivered' ? new Date() : undefined, readAt: st === 'read' ? new Date() : undefined } });
      }
    }
    res.sendStatus(200);
